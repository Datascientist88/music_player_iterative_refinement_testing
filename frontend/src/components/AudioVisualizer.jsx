...
const AudioVisualizer = () => {
  ...
  // Zustand: audio + isPlaying only
  const { audio, isPlaying } = useAudioStore((state) => ({
    audio: state.audio,
  ...
  // ✅ Properly initialize graph ONCE for this audio element
  useEffect(() => {
    if (!audio) return;

    // Create context if not existing
    if (!audioContextRef.current) {
      audioContextRef.current = new (window.AudioContext || window.webkitAudioContext)();
    }
    const audioContext = audioContextRef.current;

    // ✅ If we don't have a source yet, make one ONCE.
    if (!sourceNodeRef.current) {
      sourceNodeRef.current = audioContext.createMediaElementSource(audio);
    }

    // Always create/reuse analyser
    ...
    return () => {
      ...
      // Note: Do NOT disconnect sourceNode from audio — that's the original audio element!
      // Just let the sourceNode live as long as the audio element lives.
    };
  }, [audio]);

  // ✅ Canvas loop — same logic, uses local analyser
  useEffect(() => {
    ...
    const renderFrame = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        const radius = barHeight * (Math.min(width, height) / 2 / 255) * 0.9;
        if (radius > 1) {
   ...
   // The JSX is now just the visual part of your original component.
  return (
          ...
          <div className="visualizer">
            ...
            <div ref={svgFlowerRef} className="svg-flower">
              <svg viewBox="0 0 100 100">
                {/* All your original <defs> go here... */}
                <defs>
            ...
            <div ref={svgFlower2Ref} className="svg-flower2">
              <svg viewBox="0 0 100 100">
                {/* All your other paths and masks go here... */}
                <path
